...
def eval_trans_matrix(
    trans_dict_path="./log/pythia2opt-6.7b/glove-MX1M-iter15-d300.json", 
    eval_file_path="./data/pretrain-dataset/pythia-2-opt-6.7b-MX1K-eval",
    ...
    for s in eval_data:
        # src: source token id, e.g., pythia ids, tgt: target token id, e.g., opt-6.7b ids
        src, tgt = s[0], s[1]
...
if __name__ == '__main__':
    ...
    parser.add_argument("-m", "--one2one-matrix-path", type=str, default="./data/pythia2opt-6.7b/glove.json")
    parser.add_argument("-f", "--eval-file-path", type=str, default="./data/pretrain-dataset/pythia-2-opt-6.7b-MX1K-eval")
